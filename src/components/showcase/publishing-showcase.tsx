      ...
      <div className="p-8 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-xl shadow-slate-200/50 dark:shadow-indigo-950/20 space-y-8">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between pb-6 border-b border-slate-200 dark:border-slate-800 gap-4">
          <div>
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">LinkedIn REST API OAuth 2.0 • Hardware AES-256 Encryption</p>
          </div>
          <span className="px-3 py-1.5 rounded-full bg-emerald-50 dark:bg-emerald-950/60 text-emerald-700 dark:text-emerald-400 text-xs font-mono font-semibold border border-emerald-200 dark:border-emerald-800 self-start sm:self-auto">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
          {steps.map((step, idx) => (
            <div
              key={step.label}
              className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-950/80 border border-slate-200 dark:border-slate-800 space-y-2 relative overflow-hidden group hover:border-emerald-400 dark:hover:border-emerald-500 transition-colors shadow-xs"
            >
              <div className="flex items-center justify-between">
                <span className="text-[10px] font-bold text-emerald-700 dark:text-emerald-400 font-mono">0{idx + 1}. {step.label}</span>
                <CheckCircle2 className="w-3.5 h-3.5 text-emerald-600 dark:text-emerald-400" />
              </div>
              <div className="text-xs font-bold text-slate-900 dark:text-white">{step.time}</div>
              <div className="text-[10px] text-slate-600 dark:text-slate-400 leading-snug">{step.desc}</div>
            </div>
          ))}
        </div>

        {/* API Response Headers Console Mockup */}
        <div className="p-5 rounded-2xl bg-slate-50 dark:bg-slate-950/80 border border-slate-200 dark:border-slate-800 font-mono text-xs space-y-2 text-slate-800 dark:text-slate-200">
          <div className="text-[10px] font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400">Live API Response Console</div>
          <div className="text-emerald-700 dark:text-emerald-400 font-bold">POST https://api.linkedin.com/v2/ugcPosts</div>
          <div className="text-slate-600 dark:text-slate-400 text-[11px]">Headers: Authorization: Bearer [AES-256-GCM Encrypted Token]</div>
          <div className="text-slate-600 dark:text-slate-400 text-[11px]">Idempotency-Key: pub_idemp_9f82a1</div>
          <div className="text-indigo-700 dark:text-indigo-400 font-bold text-[11px] pt-1">
