...
export function TechArchitectureSection() {
  ...
  return (
    <section id="architecture" className="py-24 relative overflow-hidden px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto bg-slate-100/60 dark:bg-slate-900/40 border-y border-slate-200/60 dark:border-slate-800/80 rounded-3xl my-6 transition-colors duration-200">
      {/* Section Header */}
          ...
          return (
            <React.Fragment key={layer.id}>
              ...
              <div
                ...
                className={`p-6 rounded-3xl bg-white dark:bg-slate-900 border transition-all duration-300 shadow-md ${
                  isHighlighted
                    ? 'border-indigo-600 dark:border-indigo-500 ring-2 ring-indigo-500/20 shadow-xl scale-[1.01]'
              ...
              >
                <div className="flex flex-col sm:flex-row sm:items-center justify-between pb-4 border-b border-slate-200 dark:border-slate-800 gap-3 mb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 className="text-lg font-bold text-slate-900 dark:text-white">{layer.title}</h3>
                      <p className="text-xs font-mono text-indigo-600 dark:text-indigo-400 font-bold mt-0.5">{layer.tech}</p>
                    </div>
                  </div>
                  <span className="px-3 py-1 rounded-full text-xs font-mono font-bold bg-slate-100 dark:bg-slate-800 text-slate-800 dark:text-slate-200 border border-slate-200 dark:border-slate-700 self-start sm:self-auto">
                    {layer.badge}
                  </span>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                  {layer.components.map((comp, i) => (
                    <div key={i} className="p-3.5 rounded-2xl bg-slate-50 dark:bg-slate-950/80 border border-slate-200 dark:border-slate-800 space-y-1">
                      <div className="text-xs font-bold text-slate-900 dark:text-white flex items-center gap-1.5">
                        <span className="w-1.5 h-1.5 rounded-full bg-indigo-600 dark:bg-indigo-400" />
                        {comp.name}
                      </div>
                      <div className="text-[11px] text-slate-600 dark:text-slate-400 font-mono leading-tight">{comp.detail}</div>
                    </div>
                  ))}
                </div>
              </div>
              ...
              {idx < architectureLayers.length - 1 && (
                <div className="flex items-center justify-center py-1">
                  <div className="flex items-center gap-2 px-3 py-1 rounded-full bg-indigo-50 dark:bg-indigo-950/60 border border-indigo-200 dark:border-indigo-800 text-indigo-700 dark:text-indigo-300 text-[10px] font-mono font-bold">
                    <ArrowDown className="w-3.5 h-3.5 animate-bounce text-indigo-600 dark:text-indigo-400" />
      ...
      <div className="p-6 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col sm:flex-row items-center justify-between gap-4 text-xs font-mono text-slate-700 dark:text-slate-300 max-w-5xl mx-auto">
        ...
        <span className="text-slate-500 dark:text-slate-400 font-semibold">AICAS Enterprise v2.5 Architecture</span>
      </div>
    </section>
  );
}
